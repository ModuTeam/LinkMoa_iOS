...
class TodayFolderTests: XCTestCase {
    ...
    // MARK:- testTodayFolder
    /// 1000 -> 오늘의 추천 링크달 조회 성공
    func testTodayFolder_whenSucceed() throws {
        // Given
        let promise = XCTestExpectation(description: "Subscribe is executed")
        let api = LinkMoaAPI.todayFolder
        let todayFolderResponse = try JSONDecoder().decode(
            TodayFolder.self,
            from: api.sampleData
        )
        ...
        sut.rx.request(api)
            ...
            .subscribe(onSuccess: { response in
                ...
                XCTAssertEqual(response.code, todayFolderResponse.code)
                XCTAssertEqual(response.message, todayFolderResponse.message)
                XCTAssertEqual(response.result, todayFolderResponse.result)
            }, onError: { error in
                promise.fulfill()
    ...
    func testTodayFolder_whenFailure() throws {
        // Given
